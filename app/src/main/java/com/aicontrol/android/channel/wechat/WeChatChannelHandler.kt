package com.aicontrol.android.channel.wechat

import com.aicontrol.android.channel.Channel
import com.aicontrol.android.channel.ChannelHandler
...
class WeChatChannelHandler(
    ...
    private fun processInboundMessage(msg: WeChatMessage) {
        ...
        if (body.isEmpty()) {
            ...
            val app = com.aicontrol.android.AiControlApplication.instance
            val mediaTypes = msg.itemList?.filter { WeChatInbound.isMediaItem(it) }?.map {
                when (it.type) {
                    MessageItemType.IMAGE -> app.getString(com.aicontrol.android.R.string.wechat_media_image)
                    MessageItemType.VIDEO -> app.getString(com.aicontrol.android.R.string.wechat_media_video)
                    MessageItemType.FILE -> app.getString(com.aicontrol.android.R.string.wechat_media_file)
                    MessageItemType.VOICE -> app.getString(com.aicontrol.android.R.string.wechat_media_voice)
                    else -> app.getString(com.aicontrol.android.R.string.wechat_media_unknown)
                }
            } ?: emptyList()
            if (mediaTypes.isNotEmpty()) {
                XLog.i(TAG, "收到纯媒体消息: types=$mediaTypes, from=${fromUserId.takeLast(16)}")
                val tip = app.getString(com.aicontrol.android.R.string.wechat_unsupported_media, mediaTypes.joinToString("+"))
